...
## 使い方 4 — 確認ダイアログに相手の名前を出す

実行基盤の確認ダイアログは、playerId だけでは誰のことか見分けにくいことがあります。コンテンツが知っている名前を添えると、ダイアログに出してもらえます。

```ts
banPlayer({ playerId: target.id, name: target.name }, (result) => {
  /* ... */
});
```

- **名前は任意です。** 渡さなければ、これまでどおり playerId だけで確認されます
- **名前は操作者を補助するためだけに使われます。** 実行基盤は名前を確かめず、playerId との突き合わせもしません。追放されるのは常に `playerId` の相手です
- **渡すのは、コンテンツがすでに共有している名前です。** たとえばニコ生ゲームでは、`resolvePlayerInfo` でユーザーの同意を得て取得し、コンテンツ自身のメッセージで共有している名前がこれにあたります
- 通知（`onPlayerBanned` / `onPlayerUnbanned`）には名前は載りません
- 名前に対応していない実行基盤では、名前が無視されるだけで追放はそのまま行われます

実行基盤は、名前が添えられていないときに、自分が把握しているアカウント名で補うことをしません。確認ダイアログを出しては取りやめることを繰り返すだけで、ユーザーの同意なく名前を知れてしまうからです。確認ダイアログに出るのは、要求したコンテンツがすでに知っている情報だけです。
...
### `banPlayer(target, callback?)`

追放を要求する。`target` は playerId の文字列か、`{ playerId: string; name?: string }` です。`name` は確認ダイアログの補助表示にだけ使われます（[使い方 4](#使い方-4--確認ダイアログに相手の名前を出す)）。

`callback` はローカルな処理のみ記述する。`BanResult` の `reason` （拒否理由）は次のいずれかです。
