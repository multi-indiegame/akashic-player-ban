...
追放・解除の成立。全インスタンスが同一 tick で同一内容を受け取る。

**自分が要求していない追放・解除も届く。** 実行基盤が管理画面やチャット UI を持つなら、そこで確定したものも同じ経路で流れてくる（PROTOCOL.md 6.C）。`banPlayer()` を一度も呼んでいなくても届くので、**両方のハンドラを常に書いておくこと**。

たとえば「チャットで荒らした人を実行基盤側の UI で追放した」場合、コンテンツがそれを受け取らないと、締め出された相手をターン順に残したまま毎ターン待ち続けることになる。解除も同じで、受け取らないと `isBanned()` が縮まず、戻ってきた相手を拒み続けてしまう。
...
- **Scene を自由に切り替えてよい。** 通知の受信は Scene 遷移に追従して担保している。「応答が来るまで Scene を変えない」といった制約は課さない
